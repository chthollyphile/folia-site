...
>
> 链接：https://pan.quark.cn/s/6e4c6fa3bc6f
...
**Arch Linux**: 官方维护的 AUR仓库 [folia-major-bin](https://aur.archlinux.org/packages/folia-major-bin)。
...
- 一个可用的网易云 API 服务（必需）
- 一组 AI 接口配置，用于 AI 主题生成（可选）
- 想在 Web 版用酷狗或 QQ 音乐时，还要各自准备一个 API 服务；桌面版内置这两个音源，不需要单独部署

建议直接看这份按步骤写好的教程：

如果想要部署自托管的 Web 版，可以参考以下教程：

- Docker 一次部署完整 Web、音乐接口与同步服务：[/guide/deploy-docker](/guide/deploy-docker)

如果是部署在 Vercel 或其他云服务上，可以参考以下教程：
...
| 用 Docker 自托管完整 Web 服务 | [Docker 全栈部署](/guide/deploy-docker) |
