...
Windows 和 Linux 桌面版可把 Folia 主窗口放到桌面底层，作为常驻歌词壁纸。可以从`设置 > 选项 > 桌面端设置 > 壁纸模式`配置，也可以在命令面板搜索`切换壁纸模式`。

壁纸模式适合长期展示，但要注意：

- 窗口全屏和置顶在壁纸模式下没有意义，相关命令会暂时不可用。
- Windows 下窗口沉入桌面后不能直接接收键盘操作，建议用托盘菜单或遥控窗退出、恢复和控制播放。
- 透明、锁定和窗口层级由壁纸模式统一管理；切换异常时可先通过托盘恢复普通窗口。

命令面板本身的全局入口、桌面命令和快捷键见[命令面板用户指南](/guide/command-palette)。
