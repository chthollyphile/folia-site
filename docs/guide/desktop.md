...
如果你只想把当前歌词读出去，不需要控制播放，可以在 `设置 > 选项 > 连接与集成 > 歌词接口` 里打开一个更轻的接口：
...
桌面版可把 Folia 主窗口放到桌面图标下方，作为常驻歌词壁纸。可以从`设置 > 选项 > 桌面端设置 > 壁纸模式`配置，也可以从系统托盘或命令面板（搜索`切换壁纸模式`）切换，无需重启应用。

| 平台 | 状态 |
| --- | --- |
| Windows | 有专门实现，支持桌面层挂载、鼠标事件转发、多显示器几何处理和桌面结构变化后的自动恢复 |
| macOS | v0.7.4 起支持。需要在`系统设置 → 隐私与安全性 → 输入监控`中勾选 Folia 并重启应用，否则无法转发鼠标；可选`壁纸模式下自动隐藏 Dock 栏`（仅底部 Dock 生效） |
| Linux | 没有专门实现，实际效果取决于窗口管理器 |

壁纸模式适合长期展示，但要注意：

- **该模式下键盘不可用**，鼠标事件由 Folia 转发。建议用托盘菜单或遥控窗退出、恢复和控制播放。
...
- 启动后直接进入播放页
- 应用内检查更新，并选择跟随的发布通道
- 自动下载更新（Windows）；macOS 与 Linux 会检查并提示新版本，但受平台限制不做自动更新，需要手动下载完整安装包覆盖，或用包管理器（Arch 用户可走 AUR）升级
- 崩溃时保存诊断日志，并可从设置里打开日志所在目录
- Windows 卸载程序可选择是否一并清除用户数据
- 本地音频缓存目录管理
- 无法解码的音频用内置 FFmpeg 转码后播放（不改动原文件）
- 播放时阻止显示器休眠
- 语音输入时自动暂停播放（仅 Windows）
- 壁纸模式（Windows / macOS）
- 模组系统（实验性，见[模组系统](/guide/mods)）
