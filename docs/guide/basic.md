> [!TIP]
> 本说明已按 v0.7.6 的主要界面与交互更新，后续版本界面可能有所变动
...
这几个顶部入口可以在 `设置 > 选项 > 界面设置 > 顶部胶囊入口` 里按需隐藏。

你需要登录对应平台的账户，才能使用在线相关功能。部分歌曲需要账户拥有 VIP 才可以播放。

### 切换在线音乐平台

Folia 目前支持三个在线音源：网易云音乐、酷狗音乐、QQ 音乐。首页右下角的音源按钮可以展开`在线音乐平台`菜单，在这里切换当前平台，或者对没登录的平台直接扫码登录。

几点需要知道：

- 顶部的`在线`、`电台`、`专辑`展示的都是**当前平台**的内容，切换平台后会整体刷新。
- 不同平台支持的能力不一样，某个平台不支持的入口不会出现。
- Web 版的酷狗和 QQ 音乐需要各自的 API 服务；没有配置时菜单里会显示 `Web 端未配置 API`，桌面版则内置了这些服务。部署方式见[配置说明](/developer/configuration)。
- Navidrome 不属于在线音乐平台，它是独立的自托管服务，入口在 `Navi` 标签。Folia 走的是标准 Subsonic / OpenSubsonic API，所以其他 Subsonic 兼容服务端也可以试着连，详见 [Navidrome / OpenSubsonic](/guide/navidrome)。
...
|------|--------|
| 播放 / 暂停 | <kbd>Space</kbd> |
| 上一首 | <kbd>Ctrl</kbd> + <kbd>←</kbd> |
| 下一首 | <kbd>Ctrl</kbd> + <kbd>→</kbd> |
| 倒退 5 秒 | <kbd>←</kbd> |
| 快进 5 秒 | <kbd>→</kbd> |
| 切换右侧面板 | <kbd>P</kbd> |
| 隐藏进度条和右下角按钮 | <kbd>H</kbd> |
| 全屏 | <kbd>F11</kbd> |
| 打开命令面板 | <kbd>Ctrl/Cmd</kbd> + <kbd>K</kbd> |
...
当前登录的在线音乐平台账户信息，以及播放音质选择

# 两个播放视图

点击播放后可以进入两个视图之一：

- **可视化**（播放页）：专注当前这一首，13 种歌词动画配各种背景，右侧有完整面板。也就是上面介绍的界面。
- **Lattice（队列拼贴）**：把整条播放队列铺成一面海报墙，正在播放的那首会被放大并叠上走字歌词。

任意界面按 <kbd>Ctrl/Cmd</kbd> + <kbd>B</kbd> 可以在两者之间切换。默认落点在 `设置 > 选项 > 界面设置 > 播放后进入的视图` 里选，首次使用时 Folia 也会主动问一次。

Lattice 没有侧边面板，所有操作都通过命令面板完成——完整说明见 [Lattice（队列拼贴）](/guide/lattice)。

# 命令面板

在任意界面按 <kbd>Ctrl/Cmd</kbd> + <kbd>K</kbd> 可以打开命令面板；焦点不在输入框时也可按 <kbd>S</kbd>。海报墙默认会把单字符用于即时筛选，因此可在`设置 > 选项 > 交互控制`里决定海报墙上的 <kbd>S</kbd> 是筛选字符还是命令面板入口。

![alt text](../public/screenshots/guide/basic/image-12.png)

绝大多数功能都可以在这里直接执行，支持中文、英文、拼音、拼音首字母和模糊搜索。点击问号按钮可查看当前环境下的全部命令。

几个常用入口：

- <kbd>Ctrl/Cmd</kbd> + <kbd>P</kbd>：直接进队列界面，可搜索、按歌手 / 专辑筛选，并批量移除或移动。
- <kbd>Ctrl/Cmd</kbd> + <kbd>F</kbd>：筛选当前网格，配合 `--play` / `--add` 直接播放或追加筛选结果。
- <kbd>:</kbd>：执行模式，再按一个字母立刻执行（`n` 下一首、`v` 音量、`q` 队列、`d` 切换明暗）。

队列批量语法、完整命令列表、执行模式和自定义快捷键见[命令面板用户指南](/guide/command-palette)。
