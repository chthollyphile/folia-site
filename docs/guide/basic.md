> [!TIP]
> 本说明已按 v0.7.3 的主要界面与交互更新，后续版本界面可能有所变动
...
这几个顶部入口可以在 `设置 > 选项 > 通用 > 顶部胶囊入口` 里按需隐藏。
...
- Navidrome 不属于在线音乐平台，它是独立的自托管服务，入口在 `Navi` 标签。
...
绝大多数功能都可以在这里直接执行，支持中文、英文、拼音和模糊搜索。点击问号按钮可查看当前环境下的全部命令；队列批量语法、执行模式和自定义快捷键等用法见[命令面板用户指南](/guide/command-palette)。
