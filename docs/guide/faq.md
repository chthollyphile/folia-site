...
![alt text](../public/screenshots/guide/faq/image-12.png)

## 为什么切换背景没有效果，窗口显示为灰色/白色/半透明磨砂状态

![alt text](../public/screenshots/guide/faq/image-11.png)

请关闭 开启播放页原生毛玻璃背景 功能。该功能会在播放页启用原生毛玻璃背景，导致自定义背景无法显示。

另外此功能要求在Windows平台，要求系统版本为 Windows 11 22H2 以上版本，否则会降级为纯色背景。
...
指令窗口包含了应用的绝大多数功能，可以点击问号按钮查看所有可用指令。

最近使用过的 10 条指令会被记住，打开面板不输入任何内容时就会先看到它们；此外还有一份使用频次统计给常用命令加权。

按 `:` 可以进入执行模式，再按一个字母立刻执行命令（`n` 下一首、`v` 音量、`q` 队列、`d` 切换明暗……）。还可以在`设置 > 选项 > 交互控制`里录一个 `Alt + 字母` 的自定义快捷键，以及在`界面设置`里固定三条快捷命令。
...
随机播放按钮不同于其他播放器，其作用是**将当前播放队列内的歌曲随机打乱一次**，再次点击会再次打乱

## 怎么退出播放页面

鼠标悬停在左上角，点击出现的返回图标，或者点击右侧按钮展开面板，在歌曲封面上点击返回主页

![alt text](../public/screenshots/guide/faq/image-7.png)

## 为什么逐字歌词动画对不上/慢半拍

绝大多数情况下是因为歌词文件本身不是逐字歌词。这种情况下，会根据整句的持续时间来估算逐字歌词动画，因此准确度比较一般。

如果是整体偏早或偏晚，先分清是哪一种：

- 所有歌都差不多的偏移量，通常是蓝牙耳机等输出设备的延迟，用 `设置 > 选项 > 播放控制 > 全局时间偏移` 一次性补偿，调整时有实时预览。
- 只有某一首对不齐，用播放页右侧面板里的单曲歌词时间偏移。
...
此外，视觉设置中可以选择自动隐藏UI，当无操作一段时间之后，进度条与按钮都会隐藏。
...
- `Server URL` 能否从当前设备直接访问（含协议和端口，结尾不要多一个 `/`）
- 用户名和密码是否正确
- 是否有反向代理、HTTPS、跨域或端口转发问题
- 页面是 HTTPS 而服务端是 HTTP 时，浏览器会拦截混合内容
- 服务本身是否正在运行

Web 部署版本的 Folia 需要连接公网地址的实例。如果你的实例在内网，建议使用桌面版 Folia 来连接。

## 我用的不是 Navidrome，能连吗

可以试。Folia 这一块走的是标准的 **Subsonic / OpenSubsonic API**（声明版本 1.16.1），不是 Navidrome 私有接口，设置项叫 `Navidrome` 只是因为它是最常见的实现。

不过不同服务端覆盖的方法和扩展不一样，能用到哪些功能取决于你的服务端实现了什么。连接成功后，设置页的`服务器信息`会列出服务端类型、版本、`OpenSubsonic` 是否可用和支持的扩展——这是判断“为什么某个功能没有”的第一现场。详见 [Navidrome / OpenSubsonic](/guide/navidrome)。

## 队列拼贴 / Lattice 是什么，怎么打开

按 `Ctrl/Cmd + B`。它把整条播放队列铺成一面海报墙，是和播放页并列的第二个播放视图。再按一次退出。

想让点击播放后直接进它：`设置 > 选项 > 界面设置 > 播放后进入的视图`。私人 FM 下不可用。完整说明见 [Lattice（队列拼贴）](/guide/lattice)。
...
## 为什么 macOS 上打开应用会提示“已损坏”

这是因为 macOS 会拦截未签名的应用。这并不是应用真的损坏了。

你可以参考 [macOS App 已损坏指南](/guide/macos-app-damaged) 里的方法，通过手动允许或移除隔离属性来正常打开应用。
