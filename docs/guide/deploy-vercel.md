...
> [!warning]
> 注意，由于酷狗服务器的播放节点默认使用HTTP和错配的CDN证书，而PWA/安全上下文要求不能在 HTTPS 中混入 HTTP，因此相关的网络请求会自动升级到HTTPS
> 这在某些情况下，会导致酷狗播放失败。此时可以考虑手动访问 https://fs.youthandroid2.kugou.com/，然后信任此站点，有一定概率可以缓解该问题
> 若仍然出现错误，则可能是你附近的酷狗CDN的配置不再支持 HTTPS，此问题尚无较解决方法，如果持续出现此现象，请考虑使用网易云
